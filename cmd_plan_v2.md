...
**U1. Guidance-first calibration sweep.** Prime suspect for the CRPS-gap-with-matched-MSE signature: w > 1 sharpens the predictive distribution. Sweep w ∈ {1.0, 1.25, 1.5, 2.0} × DDIM steps {16, 32, 64} × samples {25} on val, per armcccc. Also check whether ẑ₀ brushes the dynamic threshold (p = 0.995) after the head removal/rescale — log clip rates.
