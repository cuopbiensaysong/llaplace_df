...
# stays fp32. Measured 1.31x on the denoiser forward. It IS a numerical change (CRPS moves
# in the 4th-5th decimal), so set it False to reproduce a pre-2026-07-28 number exactly;
# DETERMINISTIC=True also forces it off. (The set_torch() *parameter* defaults to False so
# that callers which do not pass it are unaffected; this config value is the project
# default and is on.)
ALLOW_TF32 = True
VERBOSE = False
DEBUG = False
...
# The val diagnostic is a full pass over the val loader; at 1 it runs every single epoch.
# Left at the historical 1 here and raised per campaign -- see the EVAL_* block below.
VAL_DIAG_EVERY = 1
IRREG_CHECK_EVERY = 0
EMA_COMPARE_EVERY = 0
...
# it (see w_docs/DEVELOPER_GUIDE.md §7.6).
#
# Unset (None / 0) means "use the reported protocol", i.e. the historical behaviour.
#
# 🔴 THESE ARE DELIBERATELY LEFT UNSET HERE. The cheap protocol was measured and validated
# on noaa_uk h=168 only, and it changes which epoch the trainer selects -- which is the
# epoch the tuning harness then scores. Enabling it globally would silently move checkpoint
# selection on every dataset, including physionet h=12, whose val split is FIVE windows
# (bug B19) and cannot afford a 25 -> 5 ensemble. Set them per campaign instead:
# finetune_noaa_uk.sh exports LLAPDIFF_EVAL_* and run_trial.py applies them. Before turning
# them on for a NEW (dataset, horizon), run the Spearman rho >= 0.9 rank check that
# DEVELOPER_GUIDE.md §7.6 requires.
EVAL_STEPS = None          # DDIM steps; None -> GEN_STEPS (64).      Campaign value: 16
EVAL_NUM_SAMPLES = None    # ensemble size; None -> NUM_EVAL_SAMPLES (25). Campaign value: 5
EVAL_MAX_BATCHES = 0       # cap on val batches per eval; 0 -> the whole loader. Campaign: 48
