...
> against 0.85 and exits non-zero on failure. **Since 2026-08-01 it also runs on a real
> dataset** — `llapdiff-stage1-roundtrip --dataset-key <ds> --pred <H>`, scoring that
> preset's frozen VAE on **val** — which is how Gate 0 of `CMD_UQ_RECOVERY_PLAN.md` is
> taken. Measured that day: noaa_uk h168 **0.922 PASS**, bms_air h168 **0.941 PASS**,
> crypto h100 0.657 FAIL, us_equity h100 0.674 FAIL, physionet h12 0.059 FAIL. Below the gate the pipeline cannot represent
...
|---|---|---|
| U1 guidance/DDIM calibration sweep | **runnable** | `llapdiff-u1-sweep --dataset-key <ds> --pred <H> --checkpoint <ckpt> --guidance 1.0 1.25 1.5 2.0 --steps 16 32 64 --weights ema` — evaluates on **val** (pre-registration rule) and logs the dynamic-threshold clip fraction per cell. ⚠️ **Pass `--weights ema`** (flag added 2026-08-01): this tool goes through `_load_stack`, so its historical `raw` default freezes the guidance choice on weights no other phase evaluates — measured 0.2673 (raw) vs 0.2593 (EMA) val CRPS on the `fixed_bugs` physionet incumbent at one fixed cell. The weight source is recorded per row and is part of the output filename. The plan's specific clip check: add `--dynamic-thresh-p 0.995` and read `clip_fraction_mean` (recorded per row together with the effective p). First data point (G3 cell-(d) ckpt, physionet h=12, val): clip fraction **0.23%** at p=0.995 — ẑ₀ barely brushes the threshold after the head removal |
| U2 Theorem-C analytic UQ (q_k, p_k⁰, Gaussian NLL, PIT) | **runnable** | train with `CHIRP_UQ_HEAD=True`, `DIFF_LOSS_MODE="gaussian_nll"` (config.py; both base-config, survive presets) + `--modal-type chirp --predict-type x0`; then `llapdiff-uq-eval --dataset-key <ds> --pred <H> --checkpoint <ckpt>` reports **latent** PIT calibration error/reliability/NLL/RMSE **and, by default, the data-space comparison**: the analytic law propagated through the decoder (latent Gaussian draws → decode, scored by the *unchanged* `evaluate_regression` — same masking/CRPS estimator/ensemble size) vs the sampled-diffusion baseline, with wall-clock for both (`analytic_speedup_x`; ~8× at 5 samples on the smoke, grows with DDIM steps). Flags: `--num-samples` (ensemble for BOTH arms; default 25), `--skip-sampled`, `--latent-only`, **`--weights {raw,ema}`**. Since 2026-08-01 `--latent-only` also accepts a checkpoint **without** the UQ head (a plain-MSE arm), reporting the mean-only latent metrics — `latent_rmse`/`latent_mse`/`latent_corr` vs `baseline_rmse_predict_mean` — and omitting everything needing a law; that is the Phase-1 signal-gate read of `CMD_UQ_RECOVERY_PLAN.md`. The data-space arms still require the head, and `--mean-source oneshot` requires `predict_type='x0'` (use `ddim` otherwise). ⚠️ **Pass `--weights ema`**: the tool goes through `_load_stack`, which reads `payload["model"]` — RAW weights even in `*_best_ema.pt` (bug B16) — so the default `raw` reports a raw-weight number while every other phase of this project uses EMA. Measured on an existing checkpoint: summed per-tensor `max\|raw − ema\|` = 0.696, i.e. the flag is not cosmetic. ⚠️ **Read the NLL warm-start warning below before launching.** |
