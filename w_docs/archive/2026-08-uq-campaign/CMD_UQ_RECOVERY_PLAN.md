...
>
> 🔴 **2026-08-04 — read B23 before running debugging lead 2.** At the shipped defaults the
> conditioning reaches only one of the denoiser's four consumers; `BLOCK_SUMMARY_ADALN=True`
> run **alone** is provably inert and would return a false negative. See the amended lead-2
> block in §1b.
>
> 🔴 **2026-08-06 — all three conditioning defects are fixed in code, and Phase 1a's `C` gate
> still does not open.** B23's dead pooled path is repaired (`COND_POOL_NORM_MODE="global"`,
> verified on real windows: across-window std 7.5e-08 → 0.533), B21 §6's objective is repaired
> (`SUM_DECODE_MODE="token"`, −32 % val reconstruction), and B20's permutation-invariant encoder
> is repaired (`VAE_ENTITY_ENCODE`). **The B21 repair moves `C` the wrong way: 5.7 % → 2.5 %**,
> with `B`'s best-view ceiling flat at ~14 %. Read `CMD_CAMPAIGN_STATE.md` §1b before deciding
> anything — in particular, **§1a's "re-enter 1b only after `C` clears 10 %" now has a fifth
> counter-example against `C` being the right criterion**, and the choice of criterion is the
> live question, not the conditioning.
...
2. **`block_summary_adaln = False`.** Conditioning currently reaches the trunk only via
   ...
   > 🔴 **Amended 2026-08-04 — B23. Do NOT run lead 2 alone.** `make_block_cond` pools the
   > summary through the same `pool_summary_tokens` as the pole path, and at the shipped
   > defaults (`COND_NORM_MODE="sample"` × `COND_POOL_MODE="mean"`) that pool is **exactly the
   > zero vector** — `normalize_cond_per_batch(mode="sample")` reduces over `dims=(1,)`
   > (`models/llapdiff_utils.py:567-570`), which is the axis `pool_summary_tokens` then averages
   > (`models/lapformer.py:485`). Setting `BLOCK_SUMMARY_ADALN=True` would widen the AdaLN
   > conditioning to `2H` and fill the new half with zeros: measured absmax **7.9e-08**. Pair it
   > with `COND_NORM_MODE="global"` or `COND_POOL_MODE="attn"`.
   >
   > The same zero also reaches `make_pole_cond`, so at the defaults **ρ(t), ω(t) and the
   > Theorem-C UQ law are computed from the diffusion timestep alone** — three of the four
   > conditioning paths are dead, cross-attention is the only live one. Verified on both Phase-1b
   > gate checkpoints' own `model_config`. This is the mechanism lead 2 describes, one link
   > earlier and larger than stated, and it is the **only** conditioning defect fixable from
   > stage-3 config knobs — B21 §6 and B5 both need a retrain and break stage-1/2 parity.

3. **B23's delivery knobs** (new, 2026-08-04). `COND_NORM_MODE="global"` restores the level to
   the pooled path; `COND_POOL_MODE="attn"` (`SummaryAttentionPool`, `lapformer.py:81` —
   original library code **no campaign run has ever exercised**) restores shape without the
   level. Being measured on noaa_uk h=168 under run-tag `b21_delivery` against the Phase-1b
   seeds as matched converged controls.

**Which readout tells you it worked.** `C` from `llapdiff-ridge-probe` measures what is *in*
`cond_summary` and never enters the denoiser, so it is **blind to B23 by construction** — a
successful delivery fix would leave `C` flat. Use `latent_corr` at
`llapdiff-uq-eval --latent-only --mean-source oneshot` as the primary readout (baseline
**0.1903 / 0.1931**): it reads the model at `t = T−1` where `x_t` carries nothing, and its seed
reproducibility of 1.5 % against CRPS's 0.036 band means 2 seeds decide it. Report `C` and val
CRPS alongside — see the three-axis table in `CMD_CAMPAIGN_STATE.md` §5.
